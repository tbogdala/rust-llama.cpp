...
#include <stdbool.h>
    ...
    load_model_result load_model(const char *fname, int n_ctx, int n_seed, bool mlock, bool embeddings, bool mmap, bool vocab_only, 
                                int n_gpu, int n_batch, const char *maingpu, const char *tensorsplit, float rope_freq, float rope_scale);
    ...
    void *llama_allocate_params(const char *prompt, int seed, int threads, int tokens, int top_k, float top_p, float min_p, float temp, float repeat_penalty,
                                ...
                                float mirostat_tau, bool penalize_nl, const char *logit_bias, const char *session_file, bool prompt_cache_in_memory, bool mlock, 
                                bool mmap, const char *maingpu, const char *tensorsplit, bool file_prompt_cache_ro, float rope_freq_base, float rope_freq_scale, 
                                int n_draft, const char *grammar);
